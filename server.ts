  ...
  app.post("/api/chat", async (req, res) => {
    try {
      ...
      const buildChartContext = (cd: any): string => {
        ...
        // ===== LÁ SỐ BÁT TỰ / TỨ TRỤ (hệ thứ hai — để chat luận chéo & giữ độ tin cậy) =====
        // Nhúng thẳng lá Bát Tự đã tính sẵn (App.tsx truyền kèm chartData.baZi) để chat
        // có thể đối chiếu Bát Tự ↔ Tử Vi NGAY trong hỏi-đáp, không chỉ dựa bản gốc.
        const baZiBlock = cd.baZi
          ? baZiToPromptText(cd.baZi)
          : "(Không có dữ liệu Bát Tự kèm theo — chỉ luận trên Tử Vi.)";
      ...
      // ===== KHO TRI THỨC TỬ BÌNH (BÁT TỰ) SÂU cho chat — Cấp 2 =====
      // Truy hồi mẫu Tử Bình theo Thập Thần nổi bật + ngũ hành thiên lệch của lá Bát Tự,
      // VÀ bắt từ khóa trong chính câu hỏi (query) để lấy đúng mẫu liên quan. Chỉ nạp khi
      // có dữ liệu Bát Tự (tránh phình prompt khi client cũ không gửi baZi).
      let tuBinhKnowledgeStr = "";
      if (chartData?.baZi) {
        const shiShenList: string[] = [];
        const ps = chartData.baZi.pillars;
        if (ps) {
          [ps.year, ps.month, ps.day, ps.hour].forEach((pl: any) => {
            if (pl?.shiShenGanVi) shiShenList.push(pl.shiShenGanVi);
            (pl?.shiShenZhiVi || []).forEach((x: string) => shiShenList.push(x));
          });
        }
        const dominantElements: string[] = chartData.baZi.wuXingWithHidden
          ? Object.entries(chartData.baZi.wuXingWithHidden)
              ...
              .map((e: any) => String(e[0]))
          : [];
        tuBinhKnowledgeStr = tuBinhToPromptText(
          retrieveTuBinh({ shiShen: shiShenList, elements: dominantElements, query: String(question) }, 7),
        );
      }

      // Chỉ giữ ~12 lượt gần nhất để prompt không phình vô hạn theo độ dài hội
      // thoại (chậm dần, có thể vượt context window). Các lượt cũ xa bị bỏ;
      // ngữ cảnh lá số + bản luận giải gốc vẫn luôn được nhúng đầy đủ riêng.
      const MAX_CHAT_HISTORY_TURNS = 12;
      const historyArr = (Array.isArray(history) ? history : []).slice(-MAX_CHAT_HISTORY_TURNS);
      const historyStr = historyArr.map((t: any) => `${t.role === "user" ? "Đương số" : "Minh Sư"}: ${String(t.content || "").trim()}`).join("\n\n");

      const prompt = `--- DỮ LIỆU LÁ SỐ (sao THỰC CÓ — tra cứu trực tiếp ở đây) ---\n${chartContext}\n\n${tuBinhKnowledgeStr ? `--- KHO TRI THỨC TỬ BÌNH (BÁT TỰ) SÂU — truy hồi theo câu hỏi (dùng làm "sách" để luận Bát Tự CÓ CĂN CỨ, KHÔNG bịa; trình tự Vượng–Nhược → Dụng thần → Hỷ–Kỵ) ---\n${tuBinhKnowledgeStr}\n\n` : ""}--- BẢN LUẬN GIẢI LÁ SỐ GỐC (diễn giải đã có) ---\n${String(baseInterpretation || "(chưa có bản luận giải gốc — hãy luận dựa trên dữ liệu lá số ở trên)").trim()}\n\n${historyStr ? `--- CÁC LƯỢT HỎI ĐÁP TRƯỚC ĐÓ ---\n${historyStr}\n\n` : ""}--- CÂU HỎI MỚI CỦA ĐƯƠNG SỐ ---\n${String(question).trim()}\n\nHãy tra đúng cung/sao liên quan rồi trả lời trực diện, bám đúng lá số.`;
